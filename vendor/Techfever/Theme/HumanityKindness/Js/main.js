(function($) {
	...
	$.fn.formMessage = function(options) {
		...
		var options = $.extend(defaults, options);
		var element = "form[id=" + options.form + "] table[class=form] tr[id=" + options.input + "] td[class=help] div[class=ui-widget] div[class=ui-state-active]";
		var html = '<span class="ui-icon ui-icon-check"></span>';
		var isError = false;
			...
			if ($.isArray(JsonReturn.messages) != -1) {
				...
				if (JsonReturn.messagescount == "0") {
					...
					function() {
						...
						$(this).find('p[class=selection]').each(
							function() {
