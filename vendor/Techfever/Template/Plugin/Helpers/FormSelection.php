...
class FormSelection extends AbstractHelper {
	...
	public function render(ElementInterface $element) {
		...
		if (count ( $options ) == 1) {
			...
			$attributes ['value'] = $element->getSingleValue ();
			
			return sprintf ( '<input %s%s', $this->createAttributesString ( $attributes ), $this->getInlineClosingBracket () ) . sprintf ( '<p class="selection" name="' . $attributes ['name'] . '" id="' . $attributes ['name'] . '">%s</p>', $element->getSingleText () );
		} else {
