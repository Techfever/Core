...
class FormAccordion extends AbstractFormHelper {
	/**
	...
	public function fireRender(ElementInterface $element) {
		...
		$accordion_total_column = $element->getAttribute ( 'accordion_total_column' );
		$accordion_total_column = ($accordion_total_column < 1 ? 1 : $accordion_total_column);
			...
			foreach ( $elements as $element_key => $element ) {
				...
				if ($field instanceof \Zend\Form\ElementInterface) {
					...
					if ($node == $parent) {
						...
						$css_style = null;
						if ($accordion_total_column > 1) {
							$css_style = 'width: ' . (100 / $accordion_total_column) . '%; float: left;';
						}
						$messageTemplates = $form->getMessageTemplates ( $element->getName () );
						switch ($type) {
							case 'captcha' :
								$renderElement = '				<div class="row" type="input" id="' . $name . '"' . (! empty ( $css_style ) ? ' style="' . $css_style . '"' : null) . '>' . "\n";
							...
							case 'hidden' :
								...
								break;
							case 'paragraph' :
								$renderElement = '				<div class="row" type="preview" id="' . $name . '"' . (! empty ( $css_style ) ? ' style="clear: both;"' : '') . '>' . "\n";
								$renderElement .= '					<div class="label">' . call_user_func_array ( array (
										$this->getView (),
										'formLabelDisplay' 
								), array (
								...
								break;
							case 'seperator' :
								$renderElement = '				<div class="row" id="' . $name . '"' . (! empty ( $css_style ) ? ' style="clear: both;"' : '') . '>' . "\n";
								$renderElement .= '					' . $formElement . "\n";
								$renderElement .= '				</div>' . "\n";
								break;
							case 'htmleditor' :
