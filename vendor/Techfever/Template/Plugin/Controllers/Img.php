...
class Img extends AbstractPlugin {
	...
	public function __invoke($options = array()) {
		...
		if (isset($this->options['folder'])) {
			$this->options['path'] = $this->options['folder'] . '/';

			if (isset($this->options['image'])) {
				$this->options['path'] .= $this->options['image'];
			}
		...
		if (empty($src)) {
			$this->setAttribute('src', urldecode($this->getScheme() . '://' . $this->getHost() . $this->url('Theme/Image', $this->getOption('path'), $this->getOption('options'), $this->getOption('reuseMatchedParams'))));
		}
		return '<img' . $this->htmlAttribs($this->getAttributes()) . '>';
	}
