...
class isBackend extends AbstractPlugin {
	/**
	 * Host (including port)
	 *
	 * @var string
	...
	public function __invoke() {
		...
		if (! empty ( $CurrentUri )) {
			...
			if (! empty ( $RefererUri ) && strtolower ( $RefererUri ) == strtolower ( SYSTEM_BACKEND_URI )) {
				$isBackend = true;
			}
		}
		...
		if (! $isBackend) {
			if (method_exists ( $controller, 'getPluginManager' )) {
				if ($controller->getPluginManager ()->has ( 'RefererUri' )) {
					$RefererUri = $controller->getPluginManager ()->get ( 'RefererUri' );
				} else {
					$RefererUri = new RefererUri ();
				}
				
				$RefererUri = $RefererUri->__invoke ();
				if (substr ( $RefererUri, - 1 ) == "/") {
					$RefererUri = substr ( $RefererUri, 0, (strlen ( $RefererUri ) - 1) );
				}
				if (! empty ( $RefererUri ) && strtolower ( $RefererUri ) == strtolower ( SYSTEM_BACKEND_URI )) {
