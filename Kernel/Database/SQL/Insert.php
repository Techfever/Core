...
class Insert extends AbstractSql implements SqlInterface, PreparableSqlInterface {
	...
	public function values(array $values, $flag = self::VALUES_SET) {
		...
		if (is_string($firstKey)) {
			...
			if ($flag == self::VALUES_MULTIPLE) {
				$this->multiple = true;
				foreach ($values as $value) {
					$capturevalue = array();
					foreach ($this->columns as $key => $columns) {
						if (array_key_exists($columns, $value)) {
							$capturevalue[$key] = $value[$columns];
						}
					}
					$this->values[] = $capturevalue;
				}
			} else {
	...
	public function prepareStatement(AdapterInterface $adapter, StatementContainerInterface $statementContainer) {
		...
		$sql = sprintf($this->specifications[self::SPECIFICATION_INSERT], $table, $columns, $values);
