...
namespace Report\Bonus\Summary\Controller;
...
class ActionController extends AbstractActionController {
	 ...
	 *
	 */
	protected $module = 'bonus_summary';
	/**
	 *
	 * @var Input Form
	 *     
	...
	private function ReviewData($id = null) {
		...
		if ($id > 0) {
			...
			$UserBonus = new UserBonus ( $options );
			$data_sponsor = $UserBonus->getSponsorData ();
			$data_sponsor_rebate = $UserBonus->getSponsorRebateData ();
			$data_pairing = $UserBonus->getPairingData ();
			$data_matching = $UserBonus->getMatchingData ();
			$data = array (
					'sponsor' => $data_sponsor,
					'sponsor_rebate' => $data_sponsor_rebate,
					'pairing' => $data_pairing,
					'matching' => $data_matching 
			);
		}
		return $data;
	}
