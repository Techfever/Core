...
namespace Document\Gallery\Tag\Listing\Controller;
		...
		if ($Request->isXmlHttpRequest ()) {
			...
			if (is_array ( $order ) && count ( $order ) > 0) {
				...
				foreach ( $order as $ordervalue ) {
					...
					} elseif ($table == 'content_tag_detail') {
						$orderraw [] = 'cdd.' . $column . ' ' . $arrange [1];
					} elseif ($table == 'content_tag_url') {
			...
			if (! $this->isAdminUser ()) {
				$search ['content_tag'] [] = 'cd.user_access_id = "' . $this->getUserID () . '"';
				$this->getTagObject ()->setContentUserID ( $this->getUserID () );
			}
			$listdataraw = $this->getTagObject ()->getTagListing ( $search, $order, $index, $perpage, true );
			$jsonData = array ();
			$jsonData ['Result'] = "OK";
			$jsonData ['Records'] = $listdataraw;
			$jsonData ['TotalRecordCount'] = $this->getTagObject ()->getTagListingTotal ( $search, true );
		...
		if ($Request->isXmlHttpRequest ()) {
			...
			if ($this->getTagObject ()->verifyTagID ()) {
				...
				$legend ['preview'] = $this->HrefLink ( array (
						...
						'route' => ucfirst ( $this->type ) . '/Tag/Preview',
						'params' => array (
								'action' => 'Show',
								'crypt' => $id 
						) 
