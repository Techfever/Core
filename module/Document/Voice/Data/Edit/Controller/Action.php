...
namespace Document\Voice\Data\Edit\Controller;
...
class ActionController extends AbstractActionController {
	...
	public function UpdateAction() {
		...
		if ($this->isXmlHttpRequest ()) {
			...
			if ($InputForm->isPost () && $InputForm->isValid () && $action == 'submit') {
				...
				if ($this->getDataObject ()->verifyDataID () && $this->getDataObject ()->updateDataFactory ( $data )) {
	...
	public function SearchAction() {
		...
		$SearchForm->getResponse ()->setContent ( Json::encode ( array (
				...
				'query' => $content,
				'valid' => $valid,
				'js' => '$(this).StepsSetting({ modify: "' . $cryptID . '" }); $(this).ContentEditor({ content: "content_data", action : "edit" });' 
		) ) );
