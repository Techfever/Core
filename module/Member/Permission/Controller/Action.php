...
class PermissionActionController extends AbstractActionController {
	...
	public function UpdateAction() {
		...
		$this->addJavascript ( "vendor/Techfever/Theme/" . SYSTEM_THEME . "/Js/user.permission.js", array (
				'permissionformid' => $this->convertToUnderscore ( $this->getMatchedRouteName () . '/Update', '/' ) 
		) );
		...
		if ($this->isXmlHttpRequest ()) {
			...
			if ($InputForm->isPost () && $InputForm->isValid () && $action == 'submit') {
				...
				if ($this->getUserManagement ()->verifyID ( $id, $this->rankgroup, null ) && $this->getUserPermission ()->updateUserPermission ( $id, $data )) {
		...
		if (! is_object ( $this->inputform ) || ! empty ( $id )) {
			...
			if ($id > 0) {
				if ($this->getUserManagement ()->verifyID ( $id, $this->rankgroup, null )) {
					...
					if (count ( $data ) > 0) {
						$options ['user_access_id'] = $data ['user_access_id'];
						$options ['user_permission_as'] = $data ['user_permission_as'];
