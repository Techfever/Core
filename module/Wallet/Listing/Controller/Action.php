...
namespace Wallet\Listing\Controller;
...
class ActionController extends AbstractActionController {
	...
	public function GetAction() {
		...
		if ($Request->isXmlHttpRequest ()) {
			...
			$listdataraw = $this->getUserWallet ()->getHistoryListing ( $search, $order, $index, $perpage, true );
			$jsonData = array ();
			$jsonData ['Result'] = "OK";
			$jsonData ['Records'] = $listdataraw;
			$jsonData ['TotalRecordCount'] = $this->getUserWallet ()->getHistoryListingTotal ( $search, true );
			...
			$legend ['print'] = $this->HrefLink ( array (
					...
					'attributes' => array (
							...
							'onclick' => '$(this).formDialogPreview({
												uri: "' . $this->url ()->fromRoute ( ucfirst ( str_replace ( '_', '/', $this->type ) ) . '/List', array (
									'action' => 'View',
									'query' => $id 
							) ) . '",  
