...
class RejectedActionController extends AbstractActionController {
	protected $type = 'Wallet_Withdraw';
	protected $module = 'rejected';
	...
	public function IndexAction() {
		...
		$this->addJavascript ( "vendor/Techfever/Theme/" . SYSTEM_THEME . "/Js/datatable.menu.js", array (
				...
				'datalisturi' => $this->url ()->fromRoute ( ucfirst ( str_replace ( '_', '/', $this->type ) ) . '/Rejected' ),
	...
	public function GetAction() {
		...
		if ($Request->isXmlHttpRequest ()) {
			...
			$search ['user_wallet_history'] [] = 'uwh.wallet_status_id = 2';
			if (! $this->isAdminUser ()) {
	...
	public function ManageAction() {
		...
		if ($Request->isXmlHttpRequest ()) {
			...
			$legend ['print'] = $this->HrefLink ( array (
					...
					'attributes' => array (
							'href' => '#',
							'onclick' => '$(this).formDialogPreview({
												uri: "' . $this->url ()->fromRoute ( ucfirst ( str_replace ( '_', '/', $this->type ) ) . '/Rejected', array (
									'action' => 'View',
									'query' => $id 
							) ) . '",  
												title: "' . $this->getTranslate ( "text_dialog_wallet_withdraw_view_" . $this->module . "_title" ) . '",  
											});' 
					) 
			) );
			$managedata [] = array (
